<?php

namespace Database\Seeders;

use Illuminate\Database\Console\Seeds\WithoutModelEvents;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class FundersSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        //
        DB::table('funders')->insert([
            'name' =>'بنك التضامن  ',
            'description' =>'البنك الاول في اليمن لتقديم القروض لاصحاب المشاريع الصغيرة والمتوسطة',
            'our_products'=>'ثمار مشروعي , ثمار سكني , ثمار طموحي',
            'funding_conditions'=>'ان يكون يمني الجنسية, ان لا يقل عمر العميل عن 18 عاما , توفير الضمانات المطلوبة , ',
            'required_documents'=>'بطاقة شخصية  , اثبات ملكية المشروع ',
            'our_features'=>'ضمانات سهلة وميسرة , دراسة الطلب مجانية و الاعفاء من رسوم التنفيذ.',
            'guarantees'=>'ضمانة تجارية معمدة , ضمانة تجارية غير معمدة , ضمانة ذهب',
            'financing_ceilings'=>'من 100 الف الى 10 مليون',
            'facebook_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'linkedin_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'email' =>'womensguide@gmail.com',
            'phone_number' =>'777777777',
            'funder_resource_id'=>'1',
            'status' => 1,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        DB::table('funders')->insert([
            'name' =>'بنك التضامن  ',
            'description' =>'البنك الاول في اليمن لتقديم القروض لاصحاب المشاريع الصغيرة والمتوسطة',
            'our_products'=>'ثمار مشروعي , ثمار سكني , ثمار طموحي',
            'funding_conditions'=>'ان يكون يمني الجنسية, ان لا يقل عمر العميل عن 18 عاما , توفير الضمانات المطلوبة , ',
            'required_documents'=>'بطاقة شخصية  , اثبات ملكية المشروع ',
            'our_features'=>'ضمانات سهلة وميسرة , دراسة الطلب مجانية و الاعفاء من رسوم التنفيذ.',
            'guarantees'=>'ضمانة تجارية معمدة , ضمانة تجارية غير معمدة , ضمانة ذهب',
            'financing_ceilings'=>'من 100 الف الى 10 مليون',
            'facebook_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'linkedin_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'email' =>'womensguide@gmail.com',
            'phone_number' =>'777777777',
            'funder_resource_id'=>'2',
            'status' => 1,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        DB::table('funders')->insert([
            'name' =>'بنك التضامن  ',
            'description' =>'البنك الاول في اليمن لتقديم القروض لاصحاب المشاريع الصغيرة والمتوسطة',
            'our_products'=>'ثمار مشروعي , ثمار سكني , ثمار طموحي',
            'funding_conditions'=>'ان يكون يمني الجنسية, ان لا يقل عمر العميل عن 18 عاما , توفير الضمانات المطلوبة , ',
            'required_documents'=>'بطاقة شخصية  , اثبات ملكية المشروع ',
            'our_features'=>'ضمانات سهلة وميسرة , دراسة الطلب مجانية و الاعفاء من رسوم التنفيذ.',
            'guarantees'=>'ضمانة تجارية معمدة , ضمانة تجارية غير معمدة , ضمانة ذهب',
            'financing_ceilings'=>'من 100 الف الى 10 مليون',
            'facebook_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'linkedin_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'email' =>'womensguide@gmail.com',
            'phone_number' =>'777777777',
            'funder_resource_id'=>'3',
            'status' => 1,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        DB::table('funders')->insert([
            'name' =>'بنك التضامن  ',
            'description' =>'البنك الاول في اليمن لتقديم القروض لاصحاب المشاريع الصغيرة والمتوسطة',
            'our_products'=>'ثمار مشروعي , ثمار سكني , ثمار طموحي',
            'funding_conditions'=>'ان يكون يمني الجنسية, ان لا يقل عمر العميل عن 18 عاما , توفير الضمانات المطلوبة , ',
            'required_documents'=>'بطاقة شخصية  , اثبات ملكية المشروع ',
            'our_features'=>'ضمانات سهلة وميسرة , دراسة الطلب مجانية و الاعفاء من رسوم التنفيذ.',
            'guarantees'=>'ضمانة تجارية معمدة , ضمانة تجارية غير معمدة , ضمانة ذهب',
            'financing_ceilings'=>'من 100 الف الى 10 مليون',
            'facebook_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'linkedin_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'email' =>'womensguide@gmail.com',
            'phone_number' =>'777777777',
            'funder_resource_id'=>'1',
            'status' => 1,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        DB::table('funders')->insert([
            'name' =>'بنك التضامن  ',
            'description' =>'البنك الاول في اليمن لتقديم القروض لاصحاب المشاريع الصغيرة والمتوسطة',
            'our_products'=>'ثمار مشروعي , ثمار سكني , ثمار طموحي',
            'funding_conditions'=>'ان يكون يمني الجنسية, ان لا يقل عمر العميل عن 18 عاما , توفير الضمانات المطلوبة , ',
            'required_documents'=>'بطاقة شخصية  , اثبات ملكية المشروع ',
            'our_features'=>'ضمانات سهلة وميسرة , دراسة الطلب مجانية و الاعفاء من رسوم التنفيذ.',
            'guarantees'=>'ضمانة تجارية معمدة , ضمانة تجارية غير معمدة , ضمانة ذهب',
            'financing_ceilings'=>'من 100 الف الى 10 مليون',
            'facebook_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'linkedin_account' =>'https://yemen-nic.info/ministations/detail.php?ID=10170',
            'email' =>'womensguide@gmail.com',
            'phone_number' =>'777777777',
            'funder_resource_id'=>'2',
            'status' => 1,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    }
}
